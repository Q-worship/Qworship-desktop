...
async function validateBibles() {
  const versions = ['kjv', 'nkjv', 'amp', 'msg', 'esv', 'niv', 'gn'];
  const biblesDir = path.join(process.cwd(), '../client/public/data/bibles');
  
  if (!fs.existsSync(biblesDir)) {
    console.error(`Bibles directory not found: ${biblesDir}`);
    return;
  }
  ...
  for (const version of versions) {
    ...
    if (!fs.existsSync(filePath)) {
      reports.push({ version, status: 'MISSING FILE', message: `The file ${version}.json does not exist. It was likely never imported to MongoDB.` });
      continue;
    }
    ...
    // Expected verses calc
    const expectedVerseCount = BIBLE_BOOKS_LCC.reduce((acc, book) => {
        return acc + book.verses.reduce((a, b) => a + b, 0);
    }, 0);

    reports.push({ 
      version, 
      status: emptyTexts === 0 ? 'HEALTHY' : 'WARNING', 
      totalVersesExtracted: verses.length,
      expectedBibleVerses: expectedVerseCount,
      emptyTextCount: emptyTexts,
      message: emptyTexts > 0 ? `Found ${emptyTexts} verses with completely blank text strings.` : `All exported verses securely contain text.`
    });
  }
