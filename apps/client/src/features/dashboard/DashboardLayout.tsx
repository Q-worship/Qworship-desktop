...
export const QworshipHome = (): JSX.Element => {
  ...
  // Service Items State - represents content added from different sections
  const [serviceItems, setServiceItems] = useState<
    Array<{
      id: string;
      section: "pre-service" | "warm-up" | "service" | "post-service";
      type: "song" | "bible" | "video" | "announcement" | "custom";
      title: string;
      content?: any; // Song data, bible verse, or custom content
      slides: Array<{
        id: string;
        type: "verse" | "chorus" | "custom" | "bible" | "song";
        title: string;
        content: any;
        sectionLabel?: string;
        songTitle?: string;
  ...
  // Generate flat slides array from service items for display
  // MEMOIZED to prevent new array reference on every render (which was causing SLIDES_SYNC loop)
  // Each slide includes itemId for background lookup
  const slides = useMemo(() => {
    return serviceItems.flatMap((item) =>
      item.slides.map((slide) => ({ ...slide, itemId: item.id })),
    );
  }, [serviceItems]);
  ...
  const [slideTextSize, setSlideTextSize] = useState(1.0);
  ...
  const [isAccountSettingsModalOpen, setIsAccountSettingsModalOpen] =
    useState(false);
  const [isAudioSettingsModalOpen, setIsAudioSettingsModalOpen] =
    useState(false);
  ...
  const [currentPresentationId, setCurrentPresentationId] = useState<
    string | number | null
  >(() => {
    const id = sessionStorage.getItem("qworship_current_presentation_id");
  ...
  const getCurrentItemId = () => {
    ...
    if (currentSlideData) {
      const parentItem = serviceItems.find((item) =>
        item.slides.some((s) => s.id === currentSlideData.id),
      );
  ...
  const handleSlideClick = (slide: Slide, slideIndex: number) => {
    ...
    const parentItem = serviceItems.find((item) =>
      item.slides.some((s) => s.id === slide.id),
    );
  ...
  const handleGroupSlideClick = (itemId: string) => {
    ...
    if (item && item.slides.length > 0) {
      ...
      if (item.type === "song") {
        ...
        const slideIndex = slides.findIndex((s) => s.id === firstSlide.id);
                                ...
                                Object.keys(parsedLyrics).length > 0 ? (
                                  ...
                                  <div className="space-y-4">
                                    ...
                                    <div className="text-left space-y-2 mb-4">
                                      ...
                                      <p className="text-gray-400 text-sm">
                                        Enter 3+ characters to instantly search
                                        out local songs.
                                      </p>
                                    </div>
                                          ...
                                          {filteredSongs.length > 0 ? (
                                            filteredSongs.map((song: any) => (
                                              ...
                                              >
                                                ...
                                                <div className="text-xs text-[#8356F3]">
                                                  Select
                                                </div>
                                              </div>
                                            ))
                                  ...
                                  <div className="space-y-4">
                                    ...
                                    <div>
                                      ...
                                      <div className="flex space-x-2">
                                        ...
                                        />
                                        <button className="bg-[#8356F3] hover:bg-[#7145E6] text-white px-4 py-2 rounded-lg transition-colors">
                                          Browse
                                        </button>
                                      </div>
                                    </div>
                    ...
                    <div
                      ...
                      style={(() => {
                        ...
                        if (currentlyDisplayedSlide) {
                          ...
                          const parentItem = serviceItems.find((item) =>
                            item.slides.some(
                              (s) => s.id === currentlyDisplayedSlide.id,
                            ),
                          );
                          if (parentItem) {
                            const itemBackground = getItemBackground(
                            ...
                            <div
                              ...
                              style={{
                                ...
                                color:
                                  ...
                                  "#ffffff",
                                textAlign:
                                  (editorState.textAlign as any) || "center",
                                fontWeight: editorState.isBold
                                  ? "bold"
                                  : "normal",
                                fontStyle: editorState.isItalic
                    ...
                    {activeMediaTab === "cloud" ? (
                      <CloudMediaTab
                        ...
                        onSelectMedia={(mediaAsset) => {
                          ...
                          const fileUrl = `${window.location.origin}/api/cloud-media/${mediaAsset.id}/file`;
                          console.log(
