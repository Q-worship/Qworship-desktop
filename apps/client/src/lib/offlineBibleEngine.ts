...
const BOOK_ALIASES: Record<string, string> = {
  ...
  ruth: 'Ruth', rut: 'Ruth',
  '1 samuel': '1 Samuel', '1samuel': '1 Samuel', '1sam': '1 Samuel', '1sa': '1 Samuel', 'first samuel': '1 Samuel',
  '2 samuel': '2 Samuel', '2samuel': '2 Samuel', '2sam': '2 Samuel', '2sa': '2 Samuel', 'second samuel': '2 Samuel',
  '1 kings': '1 Kings', '1kings': '1 Kings', '1ki': '1 Kings', 'first kings': '1 Kings',
  '2 kings': '2 Kings', '2kings': '2 Kings', '2ki': '2 Kings', 'second kings': '2 Kings',
  '1 chronicles': '1 Chronicles', '1chronicles': '1 Chronicles', '1ch': '1 Chronicles', 'first chronicles': '1 Chronicles',
  '2 chronicles': '2 Chronicles', '2chronicles': '2 Chronicles', '2ch': '2 Chronicles', 'second chronicles': '2 Chronicles',
  ...
  '1 corinthians': '1 Corinthians', '1corinthians': '1 Corinthians', '1cor': '1 Corinthians', '1co': '1 Corinthians', 'first corinthians': '1 Corinthians',
  '2 corinthians': '2 Corinthians', '2corinthians': '2 Corinthians', '2cor': '2 Corinthians', '2co': '2 Corinthians', 'second corinthians': '2 Corinthians',
...
function getFuse(): Fuse<string> {
  if (!_fuse) {
    _fuse = new Fuse(ALL_CANONICAL_BOOKS, { threshold: 0.4, includeScore: true, minMatchCharLength: 2 });
  }
  return _fuse;
}
...
function resolveBook(raw: string): string | null {
  ...
  if (results.length > 0 && (results[0].score ?? 1) < 0.5) return results[0].item;
  return null;
}
...
const VERSION_MAP: Record<string, BibleVersion> = {
  ...
  gn: 'gn', gnt: 'gn', 'good news': 'gn', 'good news bible': 'gn',
};
...
// ============================================================
const NAV_NEXT = /^(next|next verse|go next|forward|continue|advance|following verse|one more|read on|next one|next please)$/i;
const NAV_PREV = /^(previous|prev|go back|back|prior|last one|back one|previous please)$/i;
const NAV_NEXT_CH = /^(next chapter|chapter forward|advance.*chapter|following chapter)$/i;
const NAV_PREV_CH = /^(previous chapter|prev chapter|chapter back|last chapter|prior chapter)$/i;
const VERSION_SWITCH = /(?:show me|switch to|use|change to|in the|read in|give me)?\s*(kjv|nkjv|niv|esv|amp|msg|gn|gnt|amplified|king james|new king james|english standard|new international|good news|the message|message)\s*(version|translation|bible)?$/i;
...
const WORD_NUMS: Record<string, number> = {
  ...
  sixty: 60, seventy: 70, eighty: 80, ninety: 90, hundred: 100, first: 1, second: 2,
...
// Main parser
// ============================================================
export function parseVoiceCommand(text: string, defaultVersion: BibleVersion = 'kjv'): ParsedVoiceCommand {
  const original = text.trim();
  const normalized = convertWordNumbers(original).trim();
  const lower = normalized.toLowerCase();

  // Navigation
  if (NAV_NEXT.test(lower)) return { originalText: original, parsedReference: null, commandType: 'verse_change', navigationDirection: 'next', confidence: 1 };
  if (NAV_PREV.test(lower)) return { originalText: original, parsedReference: null, commandType: 'verse_change', navigationDirection: 'previous', confidence: 1 };
  if (NAV_NEXT_CH.test(lower)) return { originalText: original, parsedReference: null, commandType: 'chapter_change', navigationDirection: 'next', confidence: 1 };
  if (NAV_PREV_CH.test(lower)) return { originalText: original, parsedReference: null, commandType: 'chapter_change', navigationDirection: 'previous', confidence: 1 };

  // Jump to verse number: "verse 5"
  const jumpMatch = lower.match(/^(?:go to |jump to |verse )?verse (\d+)$/i);
  if (jumpMatch) return { originalText: original, parsedReference: null, commandType: 'jump_to_verse', targetVerse: parseInt(jumpMatch[1]), confidence: 1 };

  // Version switch
  const versionMatch = lower.match(VERSION_SWITCH);
  if (versionMatch) {
    const ver = resolveVersion(versionMatch[1]);
    if (ver) return { originalText: original, parsedReference: null, commandType: 'version_change', requestedVersion: ver, confidence: 1 };
  }
  ...
  for (const pattern of PATTERNS) {
    ...
    let bookRaw: string, chapter: number, verseStart: number, verseEnd: number | undefined;
    ...
    return {
      ...
      commandType: 'lookup',
      confidence: 0.95,
      parsedReference: { book, chapter, verseStart, verseEnd, version: defaultVersion },
    };
  }

  // Last resort: extract a book name from anywhere in the text and guess chapter/verse
  const strip = (s: string) => s.toLowerCase().replace(/\b(chapter|verse|go to|read|open|turn to|book of|the)\b/g, '').replace(/\s+/g, ' ').trim();
    ...
    if (wordPart.length >= 2) {
      ...
      if (book) {
        ...
        return {
          ...
          commandType: 'lookup',
          confidence: 0.7,
          parsedReference: { book, chapter, verseStart, version: defaultVersion },
        };
      }
    }
  }

  return { originalText: original, parsedReference: null, commandType: 'lookup', confidence: 0 };
}
 ...
 */
export async function searchOffline(reference: string, version: BibleVersion = 'kjv'): Promise<OfflineBibleResult | null> {
